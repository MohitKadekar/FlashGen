    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fadeIn">
            {flashcards.map((card, index) => (
                <div key={card.id || index} className="bg-gray-800/80 backdrop-blur-sm p-6 rounded-xl border border-gray-700/50 hover:border-indigo-500/50 transition-all duration-300 shadow-md hover:shadow-xl hover:-translate-y-1 group">
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <p className="text-gray-100 font-medium leading-relaxed">{card.question}</p>
                        </div>

                        <div className="pt-4 border-t border-gray-700/50">
                            <p className="text-xs text-gray-500 uppercase font-bold tracking-wider mb-1">Answer</p>
                            <p className="text-gray-300 leading-relaxed text-sm">{card.answer}</p>
                        </div>
                    </div>
                </div>
            ))}
